...
poverty_race <- poverty_race %>% 
  select(year, poverty_white, poverty_asian, poverty_black, poverty_hispanic, poverty_native, poverty_other) %>% 
  mutate(total_pov = poverty_white + poverty_asian + poverty_black + poverty_hispanic + poverty_native + poverty_other) %>% 
  group_by(year) %>% 
 # filter(year == 2016) %>% 
  gather(poverty_white, poverty_asian, poverty_black, poverty_hispanic, poverty_native, poverty_other, key = "race", value = "total_race") %>% 
    mutate(share = total_race/total_pov)
